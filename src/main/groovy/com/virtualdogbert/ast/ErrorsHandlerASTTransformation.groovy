...
public class ErrorsHandlerASTTransformation extends AbstractASTTransformation {
    ...
    public void visit(ASTNode[] nodes, SourceUnit sourceUnit) {
        ...
        if (nodes[0] instanceof AnnotationNode && nodes[1] instanceof MethodNode) {
            ...
            ListExpression commandParams = new ListExpression(getCommandParamsList(methodNode.parameters.toList()))
            MapExpression otherParams = getOtherParamsMap(methodNode.parameters.toList())

            applyToMethod(methodNode, sourceUnit, handlerName, commandParams, otherParams)
            ...
            classNode.methods.each { MethodNode methodNode ->
                ...
                applyToMethod(methodNode, sourceUnit, handlerName, commandParams, otherParams, true)
            }
     ...
     * @param commandParams the parameter passed into the annotation at the class or method level
     * @param fromClass If the annotation comes from the class level
     */
    private void applyToMethod(MethodNode methodNode, SourceUnit sourceUnit, String handler, ListExpression commandParams, MapExpression otherParams, boolean fromClass = false) {
        ...
        statements.add(0, createErrorsHandlerCall(handler, commandParams, otherParams))
     ...
     */
    private List<Expression> getCommandParamsList(List<Parameter> methodParams) {
     ...
     * @return A list of parameters that will be passed to the error handler.
     */
    private MapExpression getOtherParamsMap(List<Parameter> methodParams) {
        MapExpression otherParametersMap = new MapExpression()

        methodParams.each { Parameter parameter ->
            if (!(parameter.type.name.endsWith(CommandArtefactHandler.TYPE))) {
                otherParametersMap.addMapEntryExpression(new MapEntryExpression(new ConstantExpression(parameter.name), new VariableExpression( parameter)))
            }
        }
     ...
     */
    private Statement createErrorsHandlerCall(String handler, ListExpression commandParams, MapExpression otherParams) {
        Expression thisExpression = new VariableExpression("this")
        Expression call = new MethodCallExpression(thisExpression, handler ?: 'errorsHandler', new ArgumentListExpression(commandParams,otherParams))
